...
// Anatomies
export const anatomiesByComponent = _.mapValues(
  componentsByName,
  (components) => _.compact(_.uniqBy(_.flatMap(components, "anatomy"), "name"))
);
    ...
    _.map(
      _.get(conceptsByComponent, [componentName, conceptName]).filter(
        (concept) =>
          ...
          concept.openUIName === conceptOpenUIName
      ),
      "sourceName"
    )
  );
...
// Images
export const getImagesForComponentConcept = (
  componentOpenUIName,
  conceptOpenUIName
) =>
  _.compact(
    _.map(
      _.get(openUIConceptsByComponent, [componentOpenUIName, conceptOpenUIName])
    )
  );
...
export const getImagesForComponent = (componentOpenUIName) => {
  const images = [];
  const arr = _.map(
    _.get(openUIConceptsByComponent, componentOpenUIName),
    (val) => _.map(val, (v) => v.image)
  );
